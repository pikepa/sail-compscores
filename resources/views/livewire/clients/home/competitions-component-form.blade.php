<x-modal>
    ...
    <x-slot name="content">
        ...
        <div>
            <x-input-label class="text-indigo-700 mt-2 font-bold">Release Date</x-input-label>
            <x-text-input wire:model='released_at' class="mt-2 w-full px-4 py-2" placeholder="Optionally enter a release date.">
            </x-text-input>
            <x-input-error :messages="$errors->get('released_at')"></x-input-error>
        </div>
    ...
    <x-slot name="buttons">
        <div class="w-full pt-5 ">
            <div class="flex justify-end">
              ...
              @if($showEdit)
              <button wire:click="updateComp('{{ $comp_id }}')" type="submit" class="ml-3 inline-flex justify-center rounded-md border border-transparent bg-indigo-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Update</button>
              @else
